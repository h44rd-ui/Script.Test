...
local function getNearestAimbotGoal(position)
    ...
    for _, PositionGoal in ipairs(Goals) do
...
local function bindAimbotTouch()
    ...
    AimbotGoal.TouchConn = Ball.Touched:Connect(function()
        ...
        local LiveTPS = FindTPS()
        if LiveTPS and LiveTPS ~= CurrentTPS then
            CurrentTPS = LiveTPS
            _tpsCache = LiveTPS
            task.defer(bindAimbotTouch)
            return
        end
...
local function GetShoot()
    ...
    if Backpack then
        return Backpack:FindFirstChild("Shoot")
    end
...
local function EquipShoot()
    ...
    if Shoot.Parent == LocalPlayer.Backpack then
        Humanoid:EquipTool(Shoot)
        task.wait(0.02)
    end
    ...
    if AimbotGoal.Enabled then
        ...
        if Ball then
            ...
            if AimbotGoal.BallControl then
                ...
                if Goal then
                    local Direction = Goal - Ball.Position
                    local Distance = Direction.Magnitude

                    if Distance <= AimbotGoal.DistanceToGoal then
                        Ball.AssemblyLinearVelocity = Vector3.zero
                        AimbotGoal.BallControl = false
                        AimbotGoal.ShotDetected = false
                        AimbotGoal.LastTouchTime = 0
                    else
                        local Velocity = Direction.Unit * AimbotGoal.Speed
