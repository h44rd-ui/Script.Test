...
local function UpdateESP()
    -- Visuals must resolve the ORIGINAL game TPS every render frame.
    local _, ball = RefreshBall(true)
    ...
    local predictedPosition =
        position + ball.AssemblyLinearVelocity * PredictionTime
...
local function bindAimbotTouch()
    ...
    if ball == AimbotGoal.ConnectedBall and AimbotGoal.TouchConn then
        return
    end
    ...
    AimbotGoal.TouchConn = boundBall.Touched:Connect(function()
        if not AimbotGoal.Enabled then
            return
        end

        -- Player touch is an immediate refresh point.
        local liveBall = GetCurrentBall()

        if liveBall ~= boundBall then
            bindAimbotTouch()
            return
        end

        if not hasShootTool() then
            return
        end
        ...
        if (liveBall.Position - root.Position).Magnitude >
            AimbotGoal.DistanceToBall then
            return
        end
...
local function setTelekinesis(v)
    ...
    if v then
        ...
        if Telekinesis.OldCameraSubject
            and Telekinesis.OldCameraSubject.Parent then
...
local function RunAutoGoalGen(gen)
    while AutoGoal and _goalGen == gen do
        ...
        local GoalPosition =
            (SelectedGoal == "Goal 2")
            and Goal2Points[GoalSides["Goal 2"] or 1]
            or Goal1Points[GoalSides["Goal 1"] or 1]
        ...
        if Distance <= 2 then
            Shoot()
            GoalSides[SelectedGoal] =
                (GoalSides[SelectedGoal] == 1) and 2 or 1
        ...
        Root.CFrame = CFrame.lookAt(
            Ball.Position - Direction * 0.8 + Vector3.new(0, 0.3, 0),
            Ball.Position
        )

        task.wait(0.005)

        -- NEVER reuse the previous ball after the wait.
        Ball = GetCurrentBall()
        ...
        -- Resolve once more immediately before the shot.
        Ball = GetCurrentBall()
...
RunService.Heartbeat:Connect(function()
    ...
    -- Update the original TPS at high frequency.
    RefreshBall(true)

    Reach:step(now, "")
    GKReach:step(now, "GK:")

    -- Aimbot Goal: always use the CURRENT ball.
    if AimbotGoal.Enabled then
        ...
        if liveBall then
            if not AimbotGoal.BallControl
                and not AimbotGoal.ShotDetected then
            ...
            if AimbotGoal.BallControl then
                -- If the game replaced TPS, immediately switch to the new one.
                local currentBall = GetCurrentBall()

                if currentBall ~= AimbotGoal.ControlledBall then
                    AimbotGoal.ControlledBall = currentBall
                    AimbotGoal.ControlledTPS = GetCurrentTPS()
                ...
                local ball = AimbotGoal.ControlledBall
                ...
                if ball and ball.Parent then
                    ...
                    if goal then
                        ...
                        if diff.Magnitude <= AimbotGoal.DistanceToGoal then
                            ...
                            local _, _, goalName =
                                getNearestAimbotGoal(ball.Position)

                            if goalName then
                                GoalSides[goalName] =
                                    (GoalSides[goalName] == 1) and 2 or 1
                            end
    ...
    -- Telekinesis: refresh and control the CURRENT ball every heartbeat.
    ...
    if AutoCatch.Enabled then
        ...
        if hum and hrp and hum.Health > 0
            and now - AutoCatch.LastCatch >= AutoCatch.Cooldown then

            local ball, dist = getNearestBall()

            if ball and dist <= AutoCatch.Range then
                fireCatchBall(ball)
            end
        end
    end
...
RunService.RenderStepped:Connect(function()
    -- ESP gets the live TPS on every rendered frame.
    RefreshBall(true)
    UpdateESP()
end)
