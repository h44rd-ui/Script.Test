...
local _tpsPartCache
local _tpsTouchConn
local _tpsFolderAddedConn
local _tpsFolderRemovingConn
...
local function getLiveTPS()
    local folder = Workspace:FindFirstChild(TPS_FOLDER_HINT)
    local tps = folder and folder:FindFirstChild("TPS") or nil

    if tps ~= _tpsCache then
        _tpsCache = tps
        CurrentTPS = tps
        _tpsPartCache = nil
    end
...
local function bindLiveBallTouch()
    if _tpsTouchConn then
        _tpsTouchConn:Disconnect()
        _tpsTouchConn = nil
    end

    local tps = getLiveTPS()
    local ball = GetBallPart(tps)
    _tpsPartCache = ball

    if not ball then return end

    _tpsTouchConn = ball.Touched:Connect(function()
        -- Re-resolve immediately when the real game ball is touched/replaced.
        local liveTPS = resolveTPS()
        if liveTPS ~= _tpsCache then
            _tpsCache = liveTPS
            CurrentTPS = liveTPS
            _tpsPartCache = GetBallPart(liveTPS)
            bindLiveBallTouch()
        end

        if AimbotGoal and AimbotGoal.Enabled then
            bindAimbotTouch()
        end

        if Telekinesis and Telekinesis.Enabled then
            local camera = Workspace.CurrentCamera
            local liveBall = GetBallPart(_tpsCache)
            if camera and liveBall then
                camera.CameraSubject = liveBall
            end
        end
    end)
end

local function refreshLiveTPS()
    local oldTPS = _tpsCache
    local newTPS = resolveTPS()

    if newTPS ~= oldTPS then
        _tpsCache = newTPS
        CurrentTPS = newTPS
        _tpsPartCache = nil
        bindLiveBallTouch()
        ...
        if Telekinesis and Telekinesis.Enabled then
            local camera = Workspace.CurrentCamera
            local ball = GetBallPart(newTPS)
            if camera and ball then
                camera.CameraSubject = ball
            end
        end
    else
        local part = GetBallPart(newTPS)
        if part ~= _tpsPartCache then
            _tpsPartCache = part
            bindLiveBallTouch()
        end
    end

    return newTPS, _tpsPartCache
end
...
local function UpdateESP()
    local tps, ball = refreshLiveTPS()
    if not tps or not ball then
...
local function getNearestAimbotGoal(position)
    ...
    for i = 1, #AIMBOT_GOALS do
        ...
        if d < bestDist then
            bestDist, best = d, AIMBOT_GOALS[i]
        end
    end
    return best, bestDist
end
...
local function bindAimbotTouch()
    ...
    AimbotGoal.TouchConn = ball.Touched:Connect(function()
        ...
        -- Always use the current real TPS after a touch.
        local _, liveBall = refreshLiveTPS()
        if not liveBall then return end

        local char = LocalPlayer.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if not root then return end

        if (liveBall.Position - root.Position).Magnitude > AimbotGoal.DistanceToBall then
            return
        end
        ...
        AimbotGoal.ShotDetected     = false
        AimbotGoal.BallControl      = false
        AimbotGoal.LastBallVelocity = liveBall.AssemblyLinearVelocity
    end)
end
...
local function setTelekinesis(v)
    ...
    if Telekinesis.MobileRefs then
        applyButtonState(Telekinesis.MobileRefs, v, "TELEKINESIS\nON", "TELEKINESIS\nOFF")
    end
...
local function RunAutoGoalGen(gen)
    local shootRunning = false

    if not shootRunning then
        shootRunning = true
        spawn(function()
    ...
    while AutoGoal and _goalGen == gen do
        ...
        local _, liveBall = refreshLiveTPS()
        if not liveBall then
            continue
        end

        BallPosition = liveBall.Position
        DistanceVector = GoalPosition - BallPosition
        ...
        local PushPosition =
            BallPosition
            + Direction * math.min(2.5, Distance)
            + Vector3.new(0, 0.3, 0)
                ...
                if goal then
                    ...
                    if dist <= AimbotGoal.DistanceToGoal then
                        ...
                        ball.AssemblyLinearVelocity =
                            ball.AssemblyLinearVelocity:Lerp(
                                target,
                                AimbotGoal.Smoothness
                            )
                    end
                end
            end
...
do
    ...
    sec:Toggle({
        ...
        Callback = function(v)
            if v then
                local _, refs = createMobileButton("LuaTelekinesisMobile", "TELEKINESIS\nOFF", function()
                    setTelekinesis(not Telekinesis.Enabled)
                end)
                Telekinesis.MobileRefs = refs
                if Telekinesis.Enabled then
                    applyButtonState(refs, true, "TELEKINESIS\nON", "TELEKINESIS\nOFF")
                end
            else
