...
local function UpdateTPS()
    local newTPS = FindTPS()

    if newTPS ~= CurrentTPS then
        CurrentTPS = newTPS
        _tpsCache = newTPS
    elseif not CurrentTPS then
        _tpsCache = nil
    end
...
local function getNearestAimbotGoal(position)
    local goals = { Goal1, Goal2 }
    local best, bestDist = nil, math.huge

    for i = 1, #goals do
        local distance = (goals[i] - position).Magnitude
...
local function bindAimbotTouch()
    ...
    AimbotGoal.TouchConn = ball.Touched:Connect(function()
        ...
        -- Re-resolve the live TPS at the moment of touch.
        UpdateTPS()
        local liveBall = GetBallPart(CurrentTPS)
...
local function detectAimbotShot(ball)
    ...
    if AimbotGoal.LastTouchTime <= 0 then
        return false
    end
...
local function setTelekinesis(v)
    ...
    if v then
        ...
        if Telekinesis.MobileRefs then
            applyButtonState(Telekinesis.MobileRefs, true, "TELECINESE\nON", "TELECINESE\nOFF")
        end
        ...
        if Telekinesis.MobileRefs then
            applyButtonState(Telekinesis.MobileRefs, false, "TELECINESE\nON", "TELECINESE\nOFF")
        end
...
local function setAutoGoal(v)
    ...
    local generation = _goalGen

    spawn(function()
        local ok, err = pcall(RunAutoGoalGen, generation)
        if not ok then
...
RunService.RenderStepped:Connect(function()
    ...
    if ESPBall or ESPPrediction then
        UpdateTPS()
    end
    UpdateESP()
end)
    ...
    sec:Toggle({
        ...
        Callback = function(v)
            if v then
                ...
                if Telekinesis.Enabled then
                    applyButtonState(refs, true, "TELECINESE\nON", "TELECINESE\nOFF")
                end
            else
